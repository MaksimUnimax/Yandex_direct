...
| D-11 | Recovery after post-request delivery failure | After `DELIVERY_SEND_TARGET_NOT_READY_BEFORE_COMMIT` on a completed request, a fresh subsequent Manual command must still execute and deliver normally without replaying the failed request | PASS | 2026-08-12 governed live recovery: fresh `getRegionsTree` completed and delivered normally with `status:OK`, `HTTP 200`, `elapsed_ms:234`, request_id `644570e1-816b-441f-906d-932c29fec7d9`; it is a new request distinct from failed-delivery request `1e7bbdfc-f67e-4079-868f-95ef58db574d`, and no replay/result for the failed request appeared before this fresh result. Existing free-call `charged:true` defect remains separately tracked by E-04/DEF-04. |
