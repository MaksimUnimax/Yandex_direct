...
| J-04 | Cross-install restore | Credentials/settings restored in another unpacked identity | TEST ERROR | 2026-08-12 first focused J-04 procedure used incorrect test storage key names for report-prefix/auto-start state. Import itself returned `imported:true` and credential/global assertions passed, but the test dereferenced a non-existent wrong-key object and stopped with `TypeError: Cannot read properties of undefined (reading 'text')`. This is a test-procedure error, not product evidence; corrected rerun is required. |
