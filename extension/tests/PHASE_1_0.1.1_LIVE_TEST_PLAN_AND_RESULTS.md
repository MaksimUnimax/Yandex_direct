...
| ID | Test | Acceptance condition | Status | Actual / evidence |
|---|---|---|---|---|
| I-01 | Worker loss before irreversible request boundary | Safe recovery may retry only if request definitely never left | TEST ERROR | 2026-08-12 first controlled I-01 procedure used an incorrect harness assertion (`WS_CONTENT_READY` response was assumed to expose `ready.run.status` directly). The focused test stopped before the product acceptance assertion with `TypeError: Cannot read properties of undefined (reading 'status')`. This is a test-procedure error, not extension evidence; corrected rerun is required. |
