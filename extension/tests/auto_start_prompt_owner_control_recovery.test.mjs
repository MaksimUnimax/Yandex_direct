...
function harness({ run = null, operation = null } = {}) {
  ...
  const ctx = vm.createContext({
    ...
    getAutoStartPrompt: async (_key, { service = null } = {}) => {
      const resolved = service || 'search';
      if (prompt?.text && prompt.is_default !== true && prompt.service === resolved) return { ...structuredClone(prompt), service: resolved };
      return { text: resolved === 'wordstat' ? 'WORDSTAT DEFAULT' : 'SEARCH DEFAULT', is_default: true, service: resolved, updated_at: prompt?.updated_at || null };
    },
...
test('same start prompt is a no-op even from non-owner during live runtime', async () => {
  const h = harness({ run: activeRun(1) });
  const saved = await h.saveApi(CKEY, 'OLD', { service: 'search', tabId: 2 });
  assert.equal(saved.text, 'OLD');
