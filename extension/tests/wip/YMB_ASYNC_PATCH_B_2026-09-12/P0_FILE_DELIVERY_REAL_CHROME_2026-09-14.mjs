  ...
  function walk(dir) {
    for (const item of fs.readdirSync(dir, { withFileTypes: true })) {
      ...
      if (item.isDirectory()) walk(file);
        ...
        rows.push([path.relative(root, file).split(path.sep).join('/'), hash(fs.readFileSync(file))]);
      }
    }
  }
  walk(root);
  rows.sort((a, b) => a[0].localeCompare(b[0]));
  assert.equal(rows.length, expectedFiles);
  return hash(Buffer.from(rows.map(([p, h]) => `${h}  ${p}\n`).join('')));
}

function emit(row) {
  const line = JSON.stringify({ ...row, time: new Date().toISOString() }) + '\n';
  fs.appendFileSync(path.join(out, 'browser.jsonl'), line);
...
function memory() {
  ...
  while (changed) {
    changed = false;
    for (const [p, pp] of rows) if (owned.has(pp) && !owned.has(p)) { owned.add(p); changed = true; }
  }
  const selected = rows.filter(([p]) => owned.has(p));
  for (const [p] of selected) observedPids.add(p);
  const rss = selected.reduce((sum, row) => sum + row[2], 0);
  peak = Math.max(peak, rss);
  return { rss_kib: rss, pids: selected.map(([p]) => p) };
}

async function runCase(name, fn) {
  try {
    const details = await fn();
    emit({ case: name, status: 'PASS', ...details, memory: memory() });
  } catch (error) {
    failed += 1;
    emit({ case: name, status: 'FAIL_QUALIFICATION', error: String(error?.stack || error), release_allowed: false });
  }
}

async function getWorker() {
  workerTarget = await browser.waitForTarget(
    (target) => target.type() === 'service_worker' && target.url().startsWith('chrome-extension://') && target.url().endsWith('/phase3_service_worker_bootstrap.js'),
    { timeout: 20000 }
  );
...
async function outbox() {
  return worker.evaluate(async (key) => {
    const entry = await getConversationOutbox(key);
    return entry ? {
      id: entry.delivery_id,
      phase: entry.phase,
      paused: entry.delivery_paused === true,
      report_text: entry.report_text,
      filenames: (entry.artifact_descriptors || []).map((x) => x.filename)
    } : null;
  }, KEY);
}
...
async function resetPageAndOutbox() {
  await worker.evaluate(async (key) => {
    ...
    const data = await chrome.storage.local.get('wsmb_manual_operations');
    const map = data.wsmb_manual_operations || {};
    delete map[key];
    await chrome.storage.local.set({ wsmb_manual_operations: map });
  }, KEY);
  await page.evaluate(() => {
...
async function stage(label) {
  return worker.evaluate(async ({ key, tabId, label }) => {
    const deliveryId = `p0-real-chrome-${label}`;
    const artifactKey = `artifact-${deliveryId}`;
    const filename = `${deliveryId}.txt`;
    const descriptor = await YMBFileArtifactStore.stageTextArtifact({
      artifactKey,
      deliveryId,
      filename,
      text: `P0 ${label} browser evidence`
    });
...
try {
  ...
  assert.ok(Number.isInteger(expectedFiles) && expectedFiles > 0);
  ...
  assert.ok(pid);
  monitor = setInterval(() => { try { memory(); } catch {} }, 250);
  await getWorker();

  await worker.evaluate(() => {
    globalThis.__p0_provider_fetches = 0;
    globalThis.fetch = async () => { __p0_provider_fetches += 1; throw new Error('P0_REAL_CHROME_NO_PROVIDER_NETWORK'); };
  });

  page = await browser.newPage();
  await page.setRequestInterception(true);
  page.on('request', (request) => {
    if (request.isNavigationRequest() && request.url() === PAGE_URL) void request.respond({ status: 200, contentType: 'text/html; charset=utf-8', body: fixture });
  ...
  }, PAGE_URL), 'CONTENT_NOT_READY');
  tabId = await worker.evaluate(async (url) => (await chrome.tabs.query({})).find((x) => x.url === url)?.id, PAGE_URL);
  assert.ok(Number.isInteger(tabId));

  await worker.evaluate(async ({ key, cid, tabId }) => {
    await chrome.storage.local.set({
      wsmb_conversation_bindings: { [key]: { binding_id: 'p0-real-chrome', revision: 1, origin: 'https://chatgpt.com', conversation_id: cid, conversation_key: key } },
      wsmb_manual_modes: { [key]: true },
      ymb_service_contexts: { [key]: { active_service: 'search' } },
      wsmb_auto_send: false,
      ymb_settings_schema_version: 5
    });
    await new Promise((resolve) => chrome.tabs.sendMessage(tabId, { type: 'WS_APPLY_MANUAL_MODE', conversation_key: key, enabled: true, active_service: 'search' }, resolve));
  }, { key: KEY, cid: CID, tabId });

  await runCase('internal_protocol_never_enters_real_chrome_composer', async () => {
    await resetPageAndOutbox();
    const staged = await stage('safe');
    ...
    assert.ok(composer.includes(staged.filename));
    assert.ok(!composer.includes('SEARCH_ASYNC_BATCH_RESULT_V1'));
    assert.ok(!composer.includes('"action":"exportPage"'));
  ...
  await runCase('user_draft_conflict_durably_pauses_and_does_not_reinsert', async () => {
    await resetPageAndOutbox();
    ...
    assert.equal(beforeClear, 'МОЙ ЧЕРНОВИК');
    ...
    await delay(1800);
    const afterClear = await page.$eval('#prompt-textarea', (element) => element.value);
    const durable = await outbox();
    assert.equal(afterClear, '');
    assert.equal(durable?.id, staged.deliveryId);
  ...
  failed += 1;
  emit({ case: 'p0_real_chrome_harness', status: 'FAIL_QUALIFICATION', error: String(error?.stack || error), release_allowed: false });
} finally {
  if (monitor) clearInterval(monitor);
  if (browser) try { await browser.close(); } catch {}
  let remaining = [];
  for (const p of observedPids) {
    try {
      const stat = fs.readFileSync(`/proc/${p}/stat`, 'utf8');
      if (stat.slice(stat.lastIndexOf(')') + 2).split(' ')[0] !== 'Z') remaining.push(p);
    } catch {}
  }
