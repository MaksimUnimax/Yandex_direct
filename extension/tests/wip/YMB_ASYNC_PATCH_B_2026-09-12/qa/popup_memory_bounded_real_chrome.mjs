...
async function waitButton(page){
  await page.waitForFunction(()=>document.getElementById('searchAsyncRefresh')?.disabled===false,{timeout:20000});
}
...
async function launch(root){
  ...
  const id=sw.url().split('/')[2];return{browser,id,pid:browser.process().pid};
}

async function oldReproduction(){
  const root=fs.realpathSync(process.env.YMB_OLD_ROOT),{browser,id,pid}=await launch(root);let page;
  try{
