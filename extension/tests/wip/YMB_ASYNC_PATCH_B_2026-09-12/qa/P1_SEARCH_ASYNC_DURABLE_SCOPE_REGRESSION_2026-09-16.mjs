...
function harness(){
 ...
 const calls={bind:[],create:[],summary:[],runtime:[],exports:[],resolve:[]};let seq=0;
 ...
 const store={
  ...
  async pageItems(jobId,owner){guard(jobId,owner);return{rows:[],next_after:-1};}
 };
 const runtime={
...
test('chat handoff keeps status on the same durable job without owner bypass',async()=>{const h=harness();await manual(h,A,7,{action:'start',jobId:'job-1',queries:['q'],maxRequests:1,maxCostMicrorub:30500});complete(h,A);const r=await manual(h,B,8,{action:'status',jobId:'job-1'});assert.equal(r.request_executed,false);assert.match(r.report_text,/"job_id":"job-1"/);assert.equal(h.calls.summary.at(-1).owner,'search-folder:folder');});
test('chat handoff exports through the ordinary exporter with the same durable owner',async()=>{const h=harness();await manual(h,A,7,{action:'start',jobId:'job-1',queries:['q'],maxRequests:1,maxCostMicrorub:30500});complete(h,A);const r=await manual(h,B,8,{action:'exportPage',jobId:'job-1',after:-1,limit:25,revision:1});assert.equal(r.request_executed,false);assert.equal(h.calls.exports.length,1);assert.equal(h.calls.exports[0].owner,'search-folder:folder');assert.equal(h.calls.exports[0].folderId,'folder');});
test('chat handoff provider command uses the same durable owner for recover and step',async()=>{const h=harness();await manual(h,A,7,{action:'start',jobId:'job-1',queries:['q'],maxRequests:1,maxCostMicrorub:30500});complete(h,A);const r=await manual(h,B,8,{action:'submitN',jobId:'job-1',count:1});assert.equal(r.request_executed,true);assert.deepEqual(h.calls.runtime.slice(-2).map(x=>x.owner),['search-folder:folder','search-folder:folder']);});
test('different Search folder cannot adopt a job created under another credential scope',async()=>{const h=harness();await manual(h,A,7,{action:'start',jobId:'job-1',queries:['q'],maxRequests:1,maxCostMicrorub:30500});complete(h,A);h.state.folder='other-folder';const r=await manual(h,B,8,{action:'status',jobId:'job-1'});assert.match(r.report_text,/ASYNC_WRONG_OWNER/);assert.equal(r.request_executed,false);});
