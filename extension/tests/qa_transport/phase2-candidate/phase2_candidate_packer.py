...
def build(repo: Path, output: Path, manifest_output: Path | None = None):
    ...
    with zipfile.ZipFile(output, "r") as z:
        bad = z.testzip()
        file_entries = [i for i in z.infolist() if not i.filename.endswith("/")]
...
def verify_extraction(repo: Path, archive: Path, extract_dir: Path):
    ...
    actual_files = sorted(p for p in root.rglob("*") if p.is_file())
    actual = {}
...
def main():
    ...
    print("SOURCE_PACKAGE_IDENTITY_PASS" if args.verify_extract else "PACKAGE_BUILD_PASS")
