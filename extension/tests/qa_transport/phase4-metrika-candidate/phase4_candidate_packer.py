...
ROOT_NAME = "yandex-marketing-bridge-0.1.1-phase4-metrika-first-slice-candidate"
...
def selected_files(repo):
    ...
    if not src.is_dir() or not tests.is_dir():
        raise SystemExit("PHASE4_SOURCE_LAYOUT_MISSING")
    rows = []
...
def main():
    ...
    if args.freeze_manifest_output:
        ...
        print("PHASE4_EXACT_ARTIFACT_FREEZE_PASS")
    else:
        expected = json.loads(Path(args.verify_manifest).read_text(encoding="utf-8"))
        if expected.get("format") != FORMAT:
            raise SystemExit(f"PHASE4_MANIFEST_FORMAT_FAIL {expected.get('format')!r}")
