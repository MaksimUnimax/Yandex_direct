...
FORMAT = "YMB_PHASE4_METRIKA_EXACT_CANDIDATE_V1"
...
def verify_archive(archive: Path, manifest: dict):
    ...
    with zipfile.ZipFile(archive, "r") as z:
        ...
        for info in infos:
            ...
            data = z.read(info)
            actual_payload[rel] = {"path": rel, "bytes": len(data), "sha256": hashlib.sha256(data).hexdigest()}
    ...
    print("PHASE4_PACKAGE_EXACT_IDENTITY_PASS")
...
def stage_repository_layout(package_root: Path, stage_dir: Path, manifest: dict):
    ...
    for rel, row in sorted(expected_payload.items()):
        ...
        copied[rel] = {"path": rel, "bytes": len(data), "sha256": hashlib.sha256(data).hexdigest()}
        if copied[rel] != row:
            fail(f"PACKAGE_STAGING_BYTE_IDENTITY_FAIL {rel}")
    if copied != expected_payload:
        fail("PACKAGE_STAGING_MANIFEST_FAIL")
    print("PHASE4_PACKAGED_LAYOUT_IDENTITY_PASS")
...
def run_static_checks(node: str, src_dir: Path, tests_dir: Path):
    ...
    print(f"PHASE4_PACKAGED_SYNTAX_PASS count={len(scripts)}")
    print("PHASE4_PACKAGED_JSON_PASS count=2")
...
def run_suite(node: str, tests_dir: Path):
    ...
    print(f"PHASE4_PACKAGED_SUITE_PASS files={len(tests)}")


def main():
    parser = argparse.ArgumentParser(description="Run full source suite against exact frozen Phase 4 package bytes.")
    ...
    print("PHASE4_PACKAGED_PREDELIVERY_PREFLIGHT_PASS")
