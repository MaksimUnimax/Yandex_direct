...
def verify_archive(archive: Path, manifest: dict):
    ...
    print("PHASE3_PACKAGE_EXACT_IDENTITY_PASS")
...
def stage_repository_layout(package_root: Path, stage_dir: Path, manifest: dict):
    ...
    print("PHASE3_PACKAGED_LAYOUT_IDENTITY_PASS")
...
def run_static_checks(node: str, src_dir: Path, tests_dir: Path):
    ...
    print(f"PHASE3_PACKAGED_SYNTAX_PASS count={len(scripts)}")
    print("PHASE3_PACKAGED_JSON_PASS count=2")
...
def run_suite(node: str, tests_dir: Path):
    ...
    print(f"PHASE3_PACKAGED_SUITE_PASS files={len(tests)}")


def main():
    parser = argparse.ArgumentParser(description="Run full source suite against exact frozen Phase 3 package bytes.")
    ...
    print("PHASE3_PACKAGED_PREDELIVERY_PREFLIGHT_PASS")
