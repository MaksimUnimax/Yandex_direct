...
const encodedHostId = 'https%3Aopenscript.ru%3A443';
...
const gzipBase64 = gzipSync(Buffer.from(csv, 'utf8')).toString('base64');
const csvSha256 = createHash('sha256').update(Buffer.from(csv, 'utf8')).digest('hex');
const gzipSha256 = createHash('sha256').update(Buffer.from(gzipBase64, 'base64')).digest('hex');
...
function prepareQaExtension() {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'ymb-webmaster-v014-browser-'));
...
try {
  ...
  await workerEval(workerClient, `(() => {
    globalThis.__YMB_WM14_FETCHES = [];
    const gzipBase64 = '${gzipBase64}';
    const gzipBytes = Uint8Array.from(atob(gzipBase64), (ch) => ch.charCodeAt(0));
    globalThis.fetch = async (url, options = {}) => {
      ...
      globalThis.__YMB_WM14_FETCHES.push({
        url: target,
        method,
      ...
      if (target === 'https://api.webmaster.yandex.net/v4/user/42/hosts/${encodedHostId}') {
      ...
      if (target === '${downloadUrl}' && method === 'GET') {
        return new Response(gzipBytes, { status: 200, headers: { 'Content-Type': 'application/gzip' } });
      }
      throw new Error('WM14_CONTROLLED_BROWSER_UNEXPECTED_NETWORK:' + target);
    };
    return true;
  })()`);
  ...
  assert.equal(ui.version, 'v0.1.4');
  ...
  console.log('WM14_BROWSER_POPUP_430X560_FIVE_SERVICE_PASS');
  ...
  console.log('WM14_BROWSER_WEBMASTER_SECRET_REDACTION_PASS');
  ...
  let fetches = await workerEval(workerClient, 'globalThis.__YMB_WM14_FETCHES');
  ...
  console.log('WM14_BROWSER_WEBMASTER_CHECK_ONE_GET_PASS');

  let policy = await send(page, { type: 'YMB_GET_WEBMASTER_POLICY' });
  assert.equal(policy.ok, true);
  for (const method of allV014Methods) assert.ok(policy.policy.allowed_methods.includes(method), `missing ${method}`);
  ...
  for (const method of allV014Methods) assert.ok(policy.policy.allowed_methods.includes(method), `common Save shrank ${method}`);
  console.log('WM14_BROWSER_COMMON_SAVE_FULL_POLICY_PASS');

  const provider = await workerEval(workerClient, `(async () => {
    const hostInfo = await globalThis.YMBPhase5ProviderRuntime.executeWebmaster({ method: 'getHostInfo', hostId: '${hostId}' });
    ...
    return {
      hostInfo: hostInfo.report_envelope,
  ...
  assert.equal(provider.hostInfo.result.host_data_status, 'NOT_LOADED');
  assert.equal(provider.hostInfo.result.webmaster_data_ready, false);
  assert.equal(provider.hostInfo.result.verified, true);
  ...
  assert.equal(provider.collect.result.manifest.compression, 'GZIP');
  assert.equal(provider.collect.result.manifest.row_count, 1);
  assert.equal(provider.collect.result.manifest.downloaded_sha256, '${gzipSha256}');
  assert.equal(provider.collect.result.manifest.csv_sha256, '${csvSha256}');
  assert.notEqual(provider.collect.result.manifest.downloaded_sha256, provider.collect.result.manifest.csv_sha256);
  ...
  assert.equal(provider.job.compression, 'GZIP');
  assert.equal(provider.job.raw_csv, ${JSON.stringify(csv)});
  assert.equal(provider.job.raw_sha256, provider.job.csv_sha256);
  assert.equal(JSON.stringify(provider).includes('browser-webmaster-secret'), false);
  console.log('WM14_BROWSER_HOST_READINESS_PASS');
  console.log('WM14_BROWSER_GZIP_EXPORT_LIFECYCLE_PASS');

  fetches = await workerEval(workerClient, 'globalThis.__YMB_WM14_FETCHES');
  assert.equal(fetches.length, 9, JSON.stringify(fetches));
  const apiFetches = fetches.slice(0, 8);
  const storageFetch = fetches[8];
  ...
  assert.match(storageFetch.accept, /application\/gzip/);
  ...
  console.log('WM14_BROWSER_BOUNDARY_ACCOUNTING_PASS');
  ...
  console.log('WM14_BROWSER_BACKUP_SERVICE_ISOLATION_PASS');
  ...
  assert.equal(await page.$eval('#webmasterOauthToken', (node) => node.value), '');
  assert.equal(await page.evaluate(() => document.body.innerText.includes('browser-webmaster-secret')), false);
  console.log('WM14_BROWSER_REOPEN_SECRET_BLANK_PASS');

  console.log('WM14_CONTROLLED_REAL_YANDEX_REQUESTS=0');
  console.log('WEBMASTER_V014_BROWSER_RUNTIME_PASS');
} finally {
  await browser.close();
