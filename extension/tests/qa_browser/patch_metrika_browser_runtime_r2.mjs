...
replaceOnce(
  ...
  "  await page.waitForFunction(() => document.querySelector('#versionBadge')?.textContent?.startsWith('v'));\n  await page.waitForFunction(() => document.querySelector('#saveMetrikaCredential')?.disabled === false);\n  await page.waitForFunction(() => document.querySelector('#metrikaMaxReportDays')?.value === '366' && document.querySelector('#metrikaCost')?.value === '0 ₽');\n\n  const geometry = await page.evaluate(() => ({",
  'initial popup policy render quiescence'
);
...
console.log('PHASE4_METRIKA_BROWSER_HARNESS_R2_INITIAL_QUIESCENCE_PASS');
