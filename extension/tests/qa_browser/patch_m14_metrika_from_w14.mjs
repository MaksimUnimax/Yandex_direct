...
const mode = String(process.argv[3] || 'list').trim();
if (!file || !fs.existsSync(file)) throw new Error('Usage: node patch_m14_metrika_from_w14.mjs <proven-w14-harness> [list|summary]');
if (!['list', 'summary'].includes(mode)) throw new Error(`M14 unsupported mode: ${mode}`);
...
source = source.replaceAll('W14_METRIKA', mode === 'summary' ? 'M14_METRIKA_SUMMARY' : 'M14_METRIKA');

replaceExact('api.metrika.yandex.net', 'api-metrika.yandex.net', 'Metrika provider hostname', 4);
replaceExact(
  '{"method":"listHosts"}',
  mode === 'summary'
    ? '{"method":"getTrafficSummary","counterId":123,"dateFrom":"2026-08-01","dateTo":"2026-08-07"}'
    : '{"method":"listCounters","page":1,"perPage":1}',
  'Metrika fixture command'
);
const providerRoute = mode === 'summary'
  ? '/stat/v1/data?ids=123&metrics=ym%3As%3Avisits%2Cym%3As%3Ausers%2Cym%3As%3Apageviews&date1=2026-08-01&date2=2026-08-07'
  : '/management/v1/counters?offset=1&per_page=1';
replaceExact("req.url === '/v4/user/42/hosts'", `req.url === '${providerRoute}'`, 'Metrika provider route');
replaceExact(
  'JSON.stringify({ hosts: [] })',
  mode === 'summary'
    ? 'JSON.stringify({ totals: [12, 8, 30], sampled: false, total_rows: 1, contains_sensitive_data: false })'
    : 'JSON.stringify({ rows: 0, counters: [] })',
  'Metrika provider response'
);
...
replaceExact("providerHits[0].url === '/v4/user/42/hosts'", `providerHits[0].url === '${providerRoute}'`, 'Metrika provider assertion');
...
console.log(`M14_METRIKA_MODE=${mode}`);
