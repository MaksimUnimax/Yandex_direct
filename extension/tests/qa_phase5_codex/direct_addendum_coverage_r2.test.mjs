...
test('official D-04 backup tamper rejection and blank Direct OAuth preservation', async () => {
  ...
  assert.equal(credentials.direct.oauth_token, 'qa-direct-token');
  assert.equal(credentials.direct.client_login, 'changed-client-only');
  assert.equal(credentials.webmaster.oauth_token, before.webmaster.oauth_token);
  assert.equal(credentials.webmaster.user_id, before.webmaster.user_id);
  assert.equal(credentials.metrika.oauth_token, before.metrika.oauth_token);
  assert.equal(credentials.wordstat.api_key, before.wordstat.api_key);
  assert.equal(credentials.search.api_key, before.search.api_key);
...
test('official D-05 Direct Check zero/one campaign and complete compatibility error matrix', async () => {
  ...
  const cases = [
    [53,'invalid token','INVALID_OR_EXPIRED'],[54,'no access','NO_ACCESS'],[58,'app access','APP_ACCESS_NOT_APPROVED'],
    [152,'units','UNITS_EXHAUSTED'],[506,'parallel','CONCURRENCY_LIMIT'],[513,'account','DIRECT_ACCOUNT_MISSING'],
    [3000,'api access','NO_API_ACCESS'],[1002,'OAuth token invalid','INVALID_OR_EXPIRED'],[1002,'generic provider error','NOT_CHECKED'],[55,'not allowed','NOT_CHECKED']
  ];
...
test('official D-06 policy defaults and hard caps are exact', async () => {
  ...
  assert.equal(typeof defaults.tariff_checked_at, 'string');
  assert.equal(typeof defaults.tariff_source, 'string');
...
const objectCases = [
  { label:'D-07 listCampaigns', command:{method:'listCampaigns',campaignIds:[11],limit:7,offset:2}, route:'/campaigns', expectedFields:['Id','Name','StartDate','EndDate','Type','Status','State','Currency'], provider:{result:{Campaigns:[{Id:11,Name:'C',StartDate:'2026-08-01',EndDate:null,Type:'TEXT_CAMPAIGN',Status:'ACCEPTED',State:'ON',Currency:'RUB',Funds:999,Notification:'drop'}]}}, verify(result){assert.deepEqual(result.campaigns[0],{id:11,name:'C',start_date:'2026-08-01',end_date:null,type:'TEXT_CAMPAIGN',status:'ACCEPTED',state:'ON',currency:'RUB'});} },
  { label:'D-08 listAdGroups', command:{method:'listAdGroups',campaignIds:[11],adGroupIds:[21],limit:7,offset:2}, route:'/adgroups', expectedFields:['Id','Name','CampaignId','Status','ServingStatus','Type'], provider:{result:{AdGroups:[{Id:21,Name:'G',CampaignId:11,Status:'ACCEPTED',ServingStatus:'ELIGIBLE',Type:'TEXT_AD_GROUP',DynamicTextAdGroup:{drop:true}}]}}, verify(result){assert.deepEqual(result.ad_groups[0],{id:21,name:'G',campaign_id:11,status:'ACCEPTED',serving_status:'ELIGIBLE',type:'TEXT_AD_GROUP'});} },
  { label:'D-09 listAds', command:{method:'listAds',campaignIds:[11],adGroupIds:[21],adIds:[31],limit:7,offset:2}, route:'/ads', expectedFields:['Id','CampaignId','AdGroupId','Status','State','Type','Subtype'], provider:{result:{Ads:[{Id:31,CampaignId:11,AdGroupId:21,Status:'ACCEPTED',State:'ON',Type:'TEXT_AD',Subtype:'NONE',TextAd:{Href:'drop',Text:'drop'}}]}}, verify(result){assert.deepEqual(result.ads[0],{id:31,campaign_id:11,ad_group_id:21,status:'ACCEPTED',state:'ON',type:'TEXT_AD',subtype:'NONE'});} },
  { label:'D-10 listKeywords', command:{method:'listKeywords',campaignIds:[11],adGroupIds:[21],keywordIds:[41],limit:7,offset:2}, route:'/keywords', expectedFields:['Id','Keyword','State','Status','ServingStatus','AdGroupId','CampaignId','Bid','ContextBid','StrategyPriority'], provider:{result:{Keywords:[{Id:41,Keyword:'query',State:'ON',Status:'ACCEPTED',ServingStatus:'ELIGIBLE',AdGroupId:21,CampaignId:11,Bid:'123456',ContextBid:654321,StrategyPriority:'NORMAL',Productivity:{drop:true},StatisticsSearch:{drop:true},StatisticsNetwork:{drop:true}}]}}, verify(result){assert.deepEqual(result.keywords[0],{id:41,keyword:'query',state:'ON',status:'ACCEPTED',serving_status:'ELIGIBLE',ad_group_id:21,campaign_id:11,bid_micros:123456,context_bid_micros:654321,strategy_priority:'NORMAL'});} }
];
...
for (const c of objectCases) {
  test(`official ${c.label} exact trusted request and allowlisted response`, async () => {
    ...
    if (c.command.method === 'listKeywords') for (const forbidden of ['Productivity','StatisticsSearch','StatisticsNetwork']) assert.equal(JSON.stringify(body).includes(forbidden), false);
  });
}
...
test('official D-14 unknown transport outcome is UNKNOWN/no-retry for JSON and Reports', async () => {
  for (const command of [{method:'listCampaigns',limit:1},{method:'getCampaignPerformance',dateFrom:'2026-08-01',dateTo:'2026-08-01',limit:1}]) {
    const env = directEnv();
...
test('official D-21 all write/future/mutation surfaces reject locally with zero provider requests', async () => {
  const env = directEnv();
  const methods = ['add','update','delete','suspendCampaign','resumeCampaign','archiveCampaign','unarchiveCampaign','moderateAd','suspendAd','resumeAd','archiveAd','unarchiveAd','addKeyword','updateKeyword','deleteKeyword','suspendKeyword','resumeKeyword','setBid','setAuto','setBids','finance','payment','AgencyClients','Feeds','Strategies','SEARCH_QUERY_PERFORMANCE_REPORT'];
