/* global YMBSearchBatchRuntime, SearchBatchProtocol, YMBSearchBatchTransport */
(() => {
  "use strict";

  const baseExecuteManualBlock = executeManualBlock;
  const baseHandleAutoCommand = handleAutoCommand;
  const SEARCH = YMBServiceRegistry.SERVICES.SEARCH;
  ...
  async function batchAdmission({ job, context = {} } = {}) {
    const [settings, policy] = await Promise.all([getSettings(), getSearchPolicy()]);
    let run = null;
    ...
    const decision = policyDecisionForService(SEARCH, {
      policy,
      channel: context.channel === "autorun" ? "autorun" : "manual",
      method: "search",
      credentialState: publicCapability(settings, SEARCH).state,
  ...
  batchRuntime = YMBSearchBatchRuntime.create({
    model: YMBProviderBatchJobModel,
    protocol: SearchBatchProtocol,
    workerSessionId: `${WORKER_SESSION_ID}:search-batch`,
    uid: () => uid("search-batch"),
    estimateCostRub: async () => Number((await getSearchPolicy()).method_cost_rub?.search || 0),
    ...
    executeSearch: async (command, metadata = {}) => {
      ...
      const policy = await getSearchPolicy();
      return executeServiceCommand(SEARCH, command, {
        ...metadata,
        cost_estimate: {
  ...
  function normalizedBatchCommand(rawCommand) {
    const command = SearchBatchProtocol.normalizeCommand(rawCommand);
    if (command.action !== "start" || command.jobId) return command;
    return Object.freeze({ ...command, jobId: uid("search-batch-job") });
  }

  async function executeSearchBatchCommand(rawCommand, metadata = {}) {
  ...
  function discoverBatchManualItem(blockText) {
    const discovered = YMBSearchBatchTransport.discover(blockText, YMBBlockCommandDiscovery, YMBServiceRegistry);
    const batchItems = discovered.filter((item) => item.search_batch === true);
    if (!batchItems.length) return { batch: false, discovered };
    if (discovered.length !== 1 || batchItems.length !== 1) {
      return {
        batch: true,
        error: Object.assign(new Error("Один Manual block должен содержать ровно одну SEARCH_BATCH_API_V1 команду."), { code: "BATCH_SINGLE_COMMAND_REQUIRED" })
      };
    }
  ...
  async function executeSearchBatchManualBlock(blockText, conversationKey, sender, manualRequestToken = "") {
    ...
    if (serviceContext.active_service !== SEARCH) return { ok: false, accepted: false, code: "SERVICE_NOT_ACTIVE", error: `Активный сервис ${serviceContext.active_service}; SEARCH_BATCH_API_V1 относится к search.`, request_executed: false };
    ...
    if (currentRun?.status === WordstatAutorunModel.RUN_STATUSES.PAUSED) {
      ...
      try { YMBRunContextModel.assertServiceMatch(currentRun.active_service, SEARCH); }
      catch (error) { return { ok: false, accepted: false, code: error.code, error: error.message }; }
    }
    ...
    const operation = {
      ...
      status: "search_batch_requesting",
    ...
    if (discovered.error) {
      ...
      try {
        const result = await executeSearchBatchCommand(command, {
          channel: "manual",
          conversation_key: key,
  ...
  async function handleSearchBatchAutoCommand(message, sender) {
    const commandText = String(message?.command_text || "");
    const detected = YMBSearchBatchTransport.detect(commandText, YMBServiceRegistry);
    if (!detected?.search_batch) return baseHandleAutoCommand(message, sender);
    ...
    if (currentRun.active_service !== SEARCH) {
      return stageAutorunError(key, currentRun, senderTabId, {
        code: "SERVICE_NOT_ACTIVE",
        message: `Активный сервис ${currentRun.active_service}; SEARCH_BATCH_API_V1 относится к search.`,
    ...
    let parsed;
    try { parsed = normalizedBatchCommand(SearchBatchProtocol.parseCommand(commandText)); }
    catch (error) {
      return stageAutorunError(key, currentRun, senderTabId, {
    ...
    let result;
    try {
      result = await executeSearchBatchCommand(parsed, {
        channel: "autorun",
        conversation_key: key,
        run_id: run.run_id
      });
  ...
  async function recoverSearchBatchManualOperations() {
    const data = await storageGet(KEYS.MANUAL_OPERATIONS);
    const map = { ...(data[KEYS.MANUAL_OPERATIONS] || {}) };
    let changed = false;
    for (const [key, operation] of Object.entries(map)) {
      if (!operation || operation.status !== "search_batch_requesting") continue;
      ...
      if (operation.batch_job_id) {
        try {
          const status = await executeSearchBatchCommand({ action: "status", jobId: operation.batch_job_id }, { channel: "manual", conversation_key: key, run_id: operation.run_id || null });
          reportText = status.report_text;
          requestExecuted = Number(status.report_envelope?.progress?.outcome_unknown || 0) > 0 ? "UNKNOWN" : false;
        } catch (error) {
          reportText = formatBridgeError({ code: error.code || "BATCH_RECOVERY_ERROR", message: error.message || String(error), stage: "BATCH_RECOVERY", requestExecuted: false, service: SEARCH, channel: "manual", recoverable: true, operation: operation.batch_action ? `batch.${operation.batch_action}` : null, operationId: operation.operation_id, autorunContinues: false });
        }
      } else {
        reportText = formatBridgeError({ code: "BATCH_OPERATION_INTERRUPTED", message: "Search batch operation была прервана до подтверждённого provider request; автоматический повтор не выполняется.", stage: "BATCH_RECOVERY", requestExecuted: false, service: SEARCH, channel: "manual", recoverable: true, operation: operation.batch_action ? `batch.${operation.batch_action}` : null, operationId: operation.operation_id, autorunContinues: false });
      }
      const deliveryId = uid("delivery");
  ...
  executeManualBlock = executeSearchBatchManualBlock;
  handleAutoCommand = handleSearchBatchAutoCommand;

  void batchRuntime.recoverAll()
    .then(recoverSearchBatchManualOperations)
    .catch((error) => diagnostic("SEARCH_BATCH_RECOVERY_ERROR", { code: error?.code || "BATCH_RECOVERY_ERROR", message: error?.message || String(error) }, { level: "error" }));

  globalThis.YMBSearchBatchWorkerTransport = Object.freeze({
    runtime: batchRuntime,
    executeSearchBatchCommand
  });
})();
