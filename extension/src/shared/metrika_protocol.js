(() => {
  ...
  const COUNTER_ALLOWLIST = Object.freeze([
    "id", "name", "site", "status", "permission", "owner_login", "favorite", "type", "code_status", "activity_status"
  ]);
  ...
  function normalizeDateRange(raw) {
    const dateTo = asDate(raw.dateTo, "dateTo", localToday());
    const dateFrom = asDate(raw.dateFrom, "dateFrom", daysAgo(6));
  ...
  function normalizeCommand(raw) {
    ...
    const allowed = fieldsForMethod(method);
    ...
    if (method === "listCounters") {
      const normalized = {
        method,
        page: asInteger(raw.page, "page", { defaultValue: 1, min: 1 }),
        perPage: asInteger(raw.perPage, "perPage", { defaultValue: 100, min: 1, max: 1000 })
      };
      const permission = asEnum(raw.permission, "permission", PERMISSIONS, { optional: true });
    ...
    const group = asEnum(raw.group, "group", GROUPS, { defaultValue: "day" });
    return Object.freeze({ method, counterId, ...dates, group });
  }
  ...
  function buildRequest(command) {
    ...
    if (normalized.method === "listCounters") {
      const qs = queryString([["offset", normalized.page], ["per_page", normalized.perPage], ["permission", normalized.permission]]);
      return Object.freeze({ method: "GET", url: `${MANAGEMENT_BASE_URL}/counters?${qs}` });
    }
    if (normalized.method === "getCounter") {
      return Object.freeze({ method: "GET", url: `${MANAGEMENT_BASE_URL}/counter/${encodeURIComponent(String(normalized.counterId))}` });
    }
    const base = normalized.method === "getTrafficSummary" ? `${REPORTS_BASE_URL}/data` : `${REPORTS_BASE_URL}/data/bytime`;
    const qs = queryString([
      ["ids", normalized.counterId],
      ["metrics", FIXED_METRICS_QUERY],
      ["date1", normalized.dateFrom],
      ["date2", normalized.dateTo],
      ["group", normalized.method === "getTrafficByTime" ? normalized.group : undefined]
    ]);
  ...
  function normalizeCounter(raw) {
    ...
    for (const key of COUNTER_ALLOWLIST) {
      ...
      if (key === "id") {
        ...
        out.id = Number.isSafeInteger(number) && number > 0 ? number : null;
  ...
  function metricTriple(values) {
    const source = Array.isArray(values) ? values : [];
    return { visits: numberOrNull(source[0]), users: numberOrNull(source[1]), pageviews: numberOrNull(source[2]) };
  }
  function metricSeries(rows) {
    ...
    return {
      visits: Array.isArray(metrics[0]) ? metrics[0].map(numberOrNull) : [],
      users: Array.isArray(metrics[1]) ? metrics[1].map(numberOrNull) : [],
      pageviews: Array.isArray(metrics[2]) ? metrics[2].map(numberOrNull) : []
    };
  }
  function bytimeTotals(rawTotals) {
    const source = Array.isArray(rawTotals) ? rawTotals : [];
    const scalar = source.map((item) => Array.isArray(item) ? item[0] : item);
    return metricTriple(scalar);
  }
  ...
  function normalizeProviderResult(command, parsed) {
    ...
    if (normalized.method === "listCounters") {
      const counters = Array.isArray(parsed.counters) ? parsed.counters.map(normalizeCounter).filter(Boolean) : [];
      return { rows: Number.isFinite(Number(parsed.rows)) ? Number(parsed.rows) : counters.length, counters };
    }
    if (normalized.method === "getCounter") {
      const counter = normalizeCounter(parsed.counter || parsed);
      if (!counter) fail("INVALID_METRIKA_RESPONSE", "Yandex Metrika не вернула объект счётчика.");
      return { counter };
    }
    if (normalized.method === "getTrafficSummary") {
      return {
        counter_id: normalized.counterId,
        date_from: normalized.dateFrom,
        date_to: normalized.dateTo,
        metrics: metricTriple(parsed.totals),
        ...truthFields(parsed)
      };
    }
    return {
      ...
      series: metricSeries(parsed.data),
      totals: bytimeTotals(parsed.totals),
      ...truthFields(parsed)
    };
  }
  ...
  function safeErrorPayload(status, rawText, parsed) {
    ...
    const code = candidate?.error_code || candidate?.code || candidate?.error?.code || candidate?.errors?.[0]?.error_type || candidate?.status || (Number(status) === 420 || Number(status) === 429 ? "QUOTA" : "YANDEX_METRIKA_API_ERROR");
  ...
  globalThis.MetrikaProtocol = Object.freeze({
    PREFIX, RESULT_PREFIX, MANAGEMENT_BASE_URL, REPORTS_BASE_URL, METHODS, PERMISSIONS, GROUPS, FIXED_METRICS,
    parseCommand, normalizeCommand, buildRequest, normalizeProviderResult, safeErrorPayload,
    buildResultEnvelope, formatResultEnvelope, buildSkippedEnvelope, formatSkippedReport, commandFingerprint, isCommandText
  });
})();
