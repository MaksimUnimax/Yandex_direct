...
def build_docx():
    ...
    required = [
        'Задача работы', 'сайт в целом построен правильно', '2 415', '550', '2 965',
        ...
        if line.startswith('|'):
            ...
            if len(rows[0]) == 4 and rows[0][0] == 'Запрос':
                for row in rows[1:]:
                    ...
                    for label, val in [('Почему выбрали', row[1]), ('Что показала Алиса', row[2]), ('Что это значит для сайта', row[3])]:
