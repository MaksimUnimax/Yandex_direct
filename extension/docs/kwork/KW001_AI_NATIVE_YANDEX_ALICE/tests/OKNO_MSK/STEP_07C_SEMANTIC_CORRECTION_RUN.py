...
EXTRA_NAVIGATION_RISK = [" сайт", "сайт ", " ru", ".ru", "номер телефона", "номер ремонта", "телефоны", "официальный"]
...
EXTRA_COMPONENT_RISK = [
    "сетк", "панель для", "створк", "профиль для", "комплект для", "направляющ", "ригель", "заглушк", "узлы ",
...
STATE_OR_FRAGMENT_RISK = [
    ...
    "панорамное окно снаружи", "пластиковые окна район", "окна пластиковые район",
]
...
REHAU_DIAGNOSTIC_RISK = ["конденсат", "провис", "починить"]
...
def corrected_keep_decision_v2(row: dict[str, str]) -> tuple[str, str, str]:
    ...
    if (" или " in s or "сравнение" in s) and any(x in sources for x in {"S02", "S11"}):
        return "REVIEW", "COMPARISON_INTENT_NEEDS_SEARCH_VALIDATION", "MEDIUM"
    if "S02" in sources and any_in(s, REHAU_DIAGNOSTIC_RISK):
        return "REVIEW", "REHAU_REPAIR_OR_DIAGNOSTIC_INTENT_NEEDS_SEARCH", "MEDIUM"
...
must_not_keep = {
    "rehau окна режимы", "rehau окно конструкция", "окна rehau сайт", "окна rehau ru", "окна rehau официальный",
    "окна rehau сравнение", "окно rehau roto", "окна rehau внутри на стекле конденсат починить", "окна rehau провисли",
    "алюминиевые окна fapim", "панель для пластиковой двери", "балкон без остекления", "разрешение на остекление балкона",
    "остекления балкона форум", "правильная установка пластиковых окон", "после установки пластиковых окон",
    "окно пластиковое закрыто", "открытое пластиковое окно", "панорамное окно снаружи", "окна в рассрочку без",
    "пластиковое окно в рассрочку без", "квартира с панорамными окнами", "студия с панорамными окнами",
    "апартаменты с панорамными окнами", "панорамные окна в россии", "панорамные окна в здании", "панорамные окна лес",
    "панорамные окна на море", "панорамные окна как называются", "как сейчас выглядят поставки окон rehau",
    "какие окна лучше веко или rehau", "какие окна лучше пластиковые или алюминиевые", "демонтаж алюминиевых окон",
    "демонтаж остекления балкона", "ремонт пластиковых окон самому", "ремонт пластиковых окон руками закрывается",
    "ремонт пластиковых окон район", "ремонт пластиковых окон купить", "ремонт пластиковых окон новых",
    "ремонт пластиковых окон частный", "ремонт сетки для пластиковых окон", "французские окна название",
    "французские окна это какие", "дача с панорамными окнами",
}
...
must_keep = {
    ...
    "какие пластиковые окна лучше", "как правильно выбрать пластиковые окна", "окна rehau цена", "окна rehau москва",
}
...
summary["manual_semantic_saturation_passes"] = 4
summary["candidate_status"] = "SEMANTIC_SATURATION_CANDIDATE_OWNER_REVIEW_PENDING" if not failures else "SEMANTIC_QA_FAILED"
