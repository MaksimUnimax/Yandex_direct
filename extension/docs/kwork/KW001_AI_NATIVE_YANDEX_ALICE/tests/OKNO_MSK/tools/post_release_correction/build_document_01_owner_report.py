...
def add_inline(paragraph, s):
    ...
    for m in inline_re.finditer(s):
        if m.start()>pos:
            r=paragraph.add_run(s[pos:m.start()]); r.font.name='Liberation Sans'; r.font.size=Pt(10.0)
        token=m.group(0)
        if token.startswith('**'):
            r=paragraph.add_run(token[2:-2]); r.bold=True; r.font.name='Liberation Sans'; r.font.size=Pt(10.0)
        else:
            mm=re.match(r'\[([^\]]+)\]\(([^)]+)\)',token)
            add_hyperlink(paragraph, mm.group(1), mm.group(2))
        pos=m.end()
    if pos<len(s):
        r=paragraph.add_run(s[pos:]); r.font.name='Liberation Sans'; r.font.size=Pt(10.0)
    ...
    if line.startswith('|') and i+1 < len(lines) and re.match(r'^\|[-: |]+\|$', lines[i+1].strip()):
        ...
        for j,h in enumerate(headers):
            ...
            for p in hdr_cells[j].paragraphs:
                ...
                for r in p.runs:
                    r.bold=True; r.font.name='Liberation Sans'; r.font.size=Pt(7.8)
        ...
        for row in rows:
            ...
            for cell in cells:
                ...
                for p in cell.paragraphs:
                    ...
                    for r in p.runs:
                        r.font.name='Liberation Sans'; r.font.size=Pt(7.8)
