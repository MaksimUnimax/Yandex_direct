...
def rebuild_markdown() -> None:
    ...
    assert summary['canonical_rows'] == 2856
    assert summary['active_rows'] == 2348
    assert summary['exact_assignment_decision_rows'] == 2322
    assert summary['active_unresolved_exact_owner_rows'] == 26
    assert summary['demand_groups'] == 168

    text = MD.read_text(encoding='utf-8')
    if '## 11. Материалы, использованные в исследовании' in text and '### 6.2. Девять новых фраз' in text:
        return

    ready = section(text, 2, 3)
    clarification = section(text, 3, 4)
    mapping = section(text, 4, 5)
    checks = section(text, 5, 6)
    links = section(text, 6, 7)
    acceptance = section(text, 7, 8)
    biblio_match = re.search(r'(?m)^## 8\. .*$', text)
    if not biblio_match:
        raise RuntimeError('bibliography section missing')
    bibliography = text[biblio_match.end():].strip()
    ...
    if assigned_header not in mapping or unresolved_header not in mapping:
        raise RuntimeError('Step5A mapping blocks missing')
...
def qa_and_metadata() -> None:
    ...
    ck('md_all_9_step5a_assigned', all(x in text.casefold() for x in PHRASES9))
    ck('md_all_7_step5a_unresolved', all(x in text.casefold() for x in PHRASES7))
    ...
    dcompact = compact(dtext)
    ck('docx_title', compact('Внедрение рекомендации') in dcompact)
    ck('docx_2856', compact('2 856') in dcompact)
    ck('docx_2348', compact('2 348') in dcompact)
    ck('docx_2322', compact('2 322') in dcompact)
    ck('docx_all_9_step5a_assigned', all(compact(x) in dcompact for x in PHRASES9))
    ck('docx_all_7_step5a_unresolved', all(compact(x) in dcompact for x in PHRASES7))
    ck('docx_no_role', compact('SEO-специалиста') not in dcompact and compact('Документ №02 для') not in dcompact)

    pcompact = compact(ptext)
    ck('pdf_title', compact('Внедрение рекомендации') in pcompact)
    ck('pdf_2856', compact('2 856') in pcompact)
    ck('pdf_2348', compact('2 348') in pcompact)
    ck('pdf_2322', compact('2 322') in pcompact)
    ck('pdf_step5a_assigned_section_heading', compact('Девять новых фраз с подтверждённым точным назначением') in pcompact)
    ck('pdf_step5a_unresolved_section_heading', compact('Семь новых фраз, которые пока нельзя передавать во внедрение') in pcompact)
    ck('pdf_no_role', compact('SEO-специалиста') not in pcompact and compact('Документ №02 для') not in pcompact)
    ck('pdf_page_count_sane', 14 <= pdf_pages <= 30, pdf_pages)
    ck('provider_calls_zero', True)

    missing_pdf9 = [x for x in PHRASES9 if compact(x) not in pcompact]
    missing_pdf7 = [x for x in PHRASES7 if compact(x) not in pcompact]
    ...
    qa = {
        'schema': 'OKNO_MSK_REPORT02_STEP05A_PROPAGATION_QA_V6',
        'date': '2026-09-09',
        'status': 'PASS',
        'checks_passed': sum(x['pass'] for x in checks),
        'checks_total': len(checks),
        'checks': checks,
        'pdf_table_text_extraction_diagnostics': {
            'gate_role': 'INFORMATIONAL_ONLY__TABLE_ROWS_CLOSED_BY_REMOTE_VISUAL_READBACK',
            'step5a_assigned_phrases_not_contiguous_in_pypdf_text': missing_pdf9,
            'step5a_unresolved_phrases_not_contiguous_in_pypdf_text': missing_pdf7,
        },
        'artifacts': {
...
''', encoding='utf-8')
    ...
    manifest['report02_pdf_table_text_gate'] = 'VISUAL_READBACK_REQUIRED__PYPDF_TABLE_TEXT_EXTRACTION_INFORMATIONAL_ONLY'
    ...
    state = {
        ...
        'quality': {
            ...
            'md_docx_equivalence_for_all_16_step5a_phrases': 'PASS',
            'pdf_structural_text_gate': 'PASS',
            'pdf_table_rows': 'REMOTE_VISUAL_READBACK_PENDING',
