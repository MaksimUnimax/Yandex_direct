...
QA = ROOT / 'RESEARCH_REBUILD_POST_RELEASE_DOCUMENT_01_OWNER_REPORT_QA_2026-09-06.json'
...
def add_page_number(p):
    p.alignment=WD_ALIGN_PARAGRAPH.RIGHT; r=p.add_run('Страница '); r.font.name='Liberation Sans'; r.font.size=Pt(8)
...
def build_docx():
    ...
    for marker in ['2 840','2 313','168','75','25','8','34','16 тем','1 тема','6 случаев','2 контрольных']:
    ...
    sec.page_width=Cm(21); sec.page_height=Cm(29.7); sec.top_margin=Cm(1.65); sec.bottom_margin=Cm(1.55); sec.left_margin=Cm(1.8); sec.right_margin=Cm(1.8); sec.header_distance=Cm(.7); sec.footer_distance=Cm(.7)
    styles=doc.styles; normal=styles['Normal']; normal.font.name='Liberation Sans'; normal.font.size=Pt(10.4); normal.paragraph_format.space_after=Pt(6); normal.paragraph_format.line_spacing=1.08
    for name,size,before,after in [('Title',20,0,12),('Heading 1',15.5,14,7),('Heading 2',12.5,11,5),('Heading 3',11.2,9,4)]:
        st=styles[name]; st.font.name='Liberation Sans'; st.font.size=Pt(size); st.font.bold=True; st.paragraph_format.space_before=Pt(before); st.paragraph_format.space_after=Pt(after); st.paragraph_format.keep_with_next=True
    if 'Callout' not in styles:
        s=styles.add_style('Callout',WD_STYLE_TYPE.PARAGRAPH); s.font.name='Liberation Sans'; s.font.size=Pt(10.2); s.paragraph_format.left_indent=Cm(.35); s.paragraph_format.right_indent=Cm(.35); s.paragraph_format.space_before=Pt(5); s.paragraph_format.space_after=Pt(7)
    if 'SmallTable' not in styles:
        s=styles.add_style('SmallTable',WD_STYLE_TYPE.PARAGRAPH); s.font.name='Liberation Sans'; s.font.size=Pt(8.2); s.paragraph_format.space_after=Pt(0); s.paragraph_format.line_spacing=1.0
    hp=sec.header.paragraphs[0]; hp.text='ОКНО МОСКВА · исследование спроса в Яндексе'; hp.alignment=WD_ALIGN_PARAGRAPH.RIGHT
    ...
    while i<len(lines):
        ...
        if line.startswith('|'):
            ...
            while i<len(lines) and (lines[i].startswith('|') or not lines[i].strip()):
                if lines[i].startswith('|'): tls.append(lines[i])
                i+=1
            rows=[]
            for tl in tls:
            ...
            if len(rows[0])==4 and rows[0][0]=='Запрос':
                for row in rows[1:]:
                    ...
                    for label,val in [('Почему выбрали',row[1]),('Что показала Алиса',row[2]),('Что это изменило для сайта',row[3])]:
                ...
                for row in rows[1:]:
                    ...
                    for j,val in enumerate(row):
                        ...
                        for p in c.paragraphs: p.style='SmallTable'
                if len(rows[0])==3:
                    for row in t.rows:
                        for j,w in enumerate([.8,5.9,9.2]): set_cell_width(row.cells[j],w)
            continue
        if line.startswith('- '):
            p=doc.add_paragraph(style='Normal'); p.paragraph_format.left_indent=Cm(.45); p.paragraph_format.first_line_indent=Cm(-.25); p.paragraph_format.space_after=Pt(3); p.add_run('• '); add_runs(p,line[2:]); i+=1; continue
        if re.match(r'^\d+\. ',line):
            m=re.match(r'^(\d+)\. (.*)',line); p=doc.add_paragraph(style='Normal'); p.paragraph_format.left_indent=Cm(.5); p.paragraph_format.first_line_indent=Cm(-.32); p.paragraph_format.space_after=Pt(3); r=p.add_run(m.group(1)+'. '); r.bold=True; r.font.name='Liberation Sans'; add_runs(p,m.group(2)); i+=1; continue
        para=[line]; i+=1
        ...
        if s.startswith('**2 840 запросов →'):
            p.style='Callout'; pPr=p._p.get_or_add_pPr(); shd=OxmlElement('w:shd'); shd.set(qn('w:fill'),'F2F4F6'); pPr.append(shd)
        add_runs(p,s)
        if s.startswith('**Что сохранить:**') or s.startswith('**Как должен выглядеть результат:**'): p.paragraph_format.keep_with_next=True

    props=doc.core_properties; props.title=TITLE; props.author=''; props.last_modified_by=''; props.subject='Клиентский исследовательский отчёт'; props.comments=''
    doc.save(DOCX)

if __name__=='__main__':
    build_docx()
    print('DOCUMENT_01_DOCX_BUILD_PASS',DOCX,sha256(DOCX))
