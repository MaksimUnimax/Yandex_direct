...
- GEO district/subdomain templates: inspect visible location switching, hidden menus and redirect behaviour, especially before declaring the public GEO family exhausted.

## Current Step-11 use

This report is fresh first-party page-purpose evidence. It may be used for candidate generation and task-fit analysis, but it is not itself a page-ownership verdict. `DISCOVERED_LINK_ONLY` must not be promoted to `OPENED_READ` without a current page read.