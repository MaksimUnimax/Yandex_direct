...
v4=read(V4);assign=read(ASSIGN);dec=read(DEC);hist=read(HIST);links=read(LINKS);pairs=read(PAIRS);pmap=read(PMAP)
find=[]
...
# Recompute expected pair universe AND Step-13 search trigger/reasons from primary evidence only.
