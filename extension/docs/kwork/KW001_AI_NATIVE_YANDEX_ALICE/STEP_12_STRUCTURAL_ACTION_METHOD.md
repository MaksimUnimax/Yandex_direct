...
### M12-25 — partial correction materialization

**Failure:** a correction changed the canonical structural-unit identity but row metadata and/or declared consumers were allowed to retain the old unit state.
...
CANONICAL ID CHANGE
WAS TREATED AS
COMPLETE SEMANTIC CORRECTION
```

**False assumption:** dependent task, intent, business-scope, page-role, owner, maturity and relationship fields were assumed to update automatically or to be harmless stale context.

**Control:** a canonical semantic/structural correction is atomic at its governed semantic level:

```text
CORRECTION OVERLAY
-> NEW CANONICAL ENTITY
-> REBUILD ENTITY-DERIVED ROW METADATA
-> REBUILD IMPACTED STRUCTURAL / ACTION / LINK / PAIR OUTPUTS
-> REBUILD DECLARED DOWNSTREAM CONSUMERS
-> INDEPENDENT TARGET-CONTRACT COHERENCE QA
-> PERSIST + READBACK
-> PASS RESTORED
```
...
### Stage 14 — materialize the final canonical phrase→unit→page/action master
...
The final phrase-level authority must be generated by joining each effective final unit ID to the current canonical unit contract. Equivalent unit-derived fields include task, intent, business scope, page role, primary/supporting ownership, maturity/confidence basis and governed relationship state. An overlay that changes only the ID is not the final master.

Declare one final canonical phrase/structural representation that downstream materializers are required to consume. Earlier base assignments and correction overlays remain provenance, not competing executable authorities.
...
CORRECTED_ROWS_WITH_TARGET_UNIT_METADATA_MISMATCH = 0
IDENTIFIER_ONLY_CORRECTIONS = 0
DECLARED_DOWNSTREAM_CONSUMERS_ON_SUPERSEDED_AUTHORITY = 0
...
KW001_STEP12_CANONICAL_CORRECTION_IS_ATOMIC = true
KW001_STEP12_NEW_UNIT_ID_WITH_OLD_UNIT_METADATA_FORBIDDEN = true
KW001_STEP12_FINAL_CANONICAL_PHRASE_STRUCTURAL_MASTER_REQUIRED = true
KW001_STEP12_DOWNSTREAM_MATERIALIZERS_MUST_CONSUME_FINAL_MASTER = true
