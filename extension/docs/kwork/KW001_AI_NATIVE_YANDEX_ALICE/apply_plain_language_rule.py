...
GATE = Path('extension/docs/kwork/KW001_AI_NATIVE_YANDEX_ALICE/STEP_GOAL_OUTPUT_AND_ACCOUNTABILITY_GATE.md')
STEP12 = Path('extension/docs/kwork/KW001_AI_NATIVE_YANDEX_ALICE/tests/OKNO_MSK/STEP_12_PRE_STEP_METHOD_REVIEW_2026-08-31.md')

text = GATE.read_text(encoding='utf-8')
start_heading = '### Mandatory non-specialist summary at the end of every pre-step explanation'
next_heading = '\n---\n\n## 9. Relationship to YMB completeness rule'
if start_heading in text:
    start = text.index(start_heading)
    end = text.index(next_heading, start)
    new = '''### Mandatory plain-language summary before AND after every step
...
marker = 'KW001_PRE_STEP_SUMMARY_NUMBERS_STAY_IN_MAIN_TEXT = true\n'
add = '''KW001_PLAIN_LANGUAGE_SUMMARY_REQUIRED_BEFORE_AND_AFTER_EVERY_STEP = true
...
if add.strip() not in text:
    if marker not in text:
        raise RuntimeError('expected marker not found in step goal gate')
    text = text.replace(marker, marker + add, 1)
GATE.write_text(text, encoding='utf-8')

s = STEP12.read_text(encoding='utf-8')
...
'''
anchor = '## Owner authorization gate\n'
if plain not in s:
    if anchor not in s:
        raise RuntimeError('Step12 owner authorization anchor missing')
    s = s.replace(anchor, plain + anchor, 1)
STEP12.write_text(s, encoding='utf-8')

print('PLAIN_LANGUAGE_RULE_PATCHED')
