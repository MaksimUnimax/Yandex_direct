...
if (packages.length !== 47 || clarifications.length !== 40 || relations.length !== 14 || acceptance.length !== 7) {
...
const findings = [
  ...
  ["Контентный резерв", "Подтверждены 6 добавлений раздела/FAQ и 2 расширения существующей страницы.", "8 единиц с доказанным пробелом содержания.", "Выполнять только семь полностью готовых заданий; одна задача требует бизнес-детали."],
  ...
  ["Внедрение", "47 заданий: 7 готово, 1 бизнес-деталь, 6 точных мест, 4 перепроверки, 19 назначений, 9 без изменения, 1 отложено.", "Сводный пакет ссылок разложен на уникальные пары.", "Передавать исполнителю только готовые строки; остальные обрабатывать по своему статусу."],
];
...
styleTitle(verifySheet, "Приёмка готовых изменений", "Сначала принять 7 готовых заданий по точным критериям; затем использовать 6 общих классов измерения без обещания uplift.", "G");
