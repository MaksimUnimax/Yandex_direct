# MK01 STEP 09 — TASK-FIRST CLUSTERING

## PURPOSE
Group the accepted semantic core by the user's expected result/task and current-domain meaning, not by superficial token overlap.

## WHY THIS STEP EXISTS
KW-001 showed that visible words can pull phrases into the wrong group, generic methodology can erase necessary domain rules, arbitrary cluster counts distort semantics, and ID-only fixes can leave stale derived fields.

## INPUTS
Frozen active semantic set, resolved/remaining uncertainty, current site/business/domain profile and bounded Search evidence where acquired.

## REQUIRED EVIDENCE
Complete phrase text; current business/domain profile; accepted semantic state; user-result/task cues; exact Search observations only where relevant.
