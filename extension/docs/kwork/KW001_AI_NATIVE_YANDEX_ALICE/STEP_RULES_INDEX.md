...
| Step 9 | Ordinary Yandex Search validation | **UNVALIDATED AS PERMANENT METHOD** | Fresh Search evidence/sampling/preservation/page-boundary research required before reuse. |
| Step 10 | User-task / Search clustering | **APPROVED / ACTIVE** | `STEP_10_CLUSTERING_GRANULARITY_METHOD.md` + `STEP_10_TASK_FIRST_SORTING_DECISION_METHOD.md` + `STEP_10_SORTING_AND_QA_METHOD.md` — reusable task-first core + scoped current domain profile/constraints. |
...
| Step 12 | Structural/content-routing actions | **APPROVED / ACTIVE** | `STEP_12_STRUCTURAL_ACTION_METHOD.md` + companion Step12 gates — action cannot prove itself; current-site/reuse/business/demand evidence before CREATE; local fix != global coherence; affected downstream state must be rebuilt after material upstream change. |
...
| Step 16 | AI-search evidence acquisition | **UNVALIDATED AS PERMANENT METHOD** | Fresh current provider/GenSearch/Alice evidence-method review required; private-access policy governs optional owned evidence. |
...
| Step 19 | Client deliverables | **UNVALIDATED AS PERMANENT METHOD** | Must research/map accepted evidence to the sold client deliverable and preserve uncertainty/claim boundaries. |
...
4. IF PARTIAL/UNVALIDATED -> DO FRESH METHOD RESEARCH BEFORE EXECUTION.
