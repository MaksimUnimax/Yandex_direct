...
| Step 3 | Wordstat/provider acquisition | **APPROVED / ACTIVE** | `STEP_METHOD_REVIEW_AND_LESSONS_LEDGER.md` + `BRIDGE_EVIDENCE_PERSISTENCE_GATE.md` — provider success is not collection completion; save/readback before next acquisition. |
...
| Step 5 | Targeted second acquisition / expansion | **PARTIALLY DEFINED / NOT YET UNIVERSALLY VALIDATED** | Existing lessons may guide design; fresh method review required before material reuse. |
...
| Step 8 | Freeze Search-stage semantic set | **APPROVED / ACTIVE** | `STEP_08_SEARCH_STAGE_FREEZE_METHOD.md` — evaluation dimension != evidence route; only executable evidence routes may govern unresolved states. |
...
| **Step 20** | **Final QA / release assurance** | **APPROVED / ACTIVE AFTER OWNER-DIRECTED EXTERNAL METHOD AUDIT + CORRECTION** | **`STEP_20_FINAL_QA_AND_RELEASE_ASSURANCE_METHOD.md` + `STEP_20_RECIPIENT_ACCEPTANCE_DEPTH_GATE.md` + `STEP_20_CLIENT_REPORT_OWNER_VALUE_AND_PLAIN_LANGUAGE_GATE.md` + `STEP_20_CLIENT_REPORT_RESULT_SURFACE_SCOPE_AND_ACTIONABILITY_GATE.md` + `STEP_20_CLIENT_REPORT_COMMISSIONER_NARRATIVE_AND_DEAI_GATE.md` + `STEP_20_REPORT_SPECIFIC_ACCEPTANCE_ROUTING.md`** — physical, semantic and product QA remain distinct; each promised artifact needs recipient-specific depth/language/usability QA; client wording must distinguish the observed result surface from the search process; one tested surface gets one client-facing name; a complete selected verification set must be explained as sufficient for its declared purpose rather than mislabeled as incomplete; exact canonical counts replace vague scope wording; and exhaustive no-change routing must not overwhelm the main action-oriented report. Package-wide PASS != recipient-specific PASS; analyst assurance != owner/commissioner acceptance. Report-specific routing is mandatory: Report №01 customer-research rules must not be imposed wholesale on Report №02 specialist implementation guidance, and Report №03 requires its own review before inheritance. |
