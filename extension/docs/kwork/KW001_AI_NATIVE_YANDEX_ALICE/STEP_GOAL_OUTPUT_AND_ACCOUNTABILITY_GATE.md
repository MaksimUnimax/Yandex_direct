...
B. REMAINING WORK ACROSS THE WHOLE KWORK
= the complete remaining sequence from the current point to final client delivery/acceptance.

C. GOAL OF THE CURRENT STEP
= the single concrete result this step is supposed to achieve.

D. WHAT THIS STEP SOLVES
= which uncertainty, missing evidence, decision or production requirement this step closes.

E. REQUIRED OUTPUT OF THE CURRENT STEP
...
ChatGPT may not jump directly into methodology, provider syntax, SEO terminology, source links or execution before establishing A–E.

If A–E are not stated clearly, the step is not ready for research or execution.
...
## 2. Then research the method

Only after the goal-first block is complete may ChatGPT research how to perform the step.
...
## 3. Every concrete job step must embed this gate
...
If the concrete step does not contain the goal/output block:
...
## 4. Technical activity never substitutes for the step goal
...
## 5. Mandatory end-of-step report
...
## 6. Mandatory transition decision
...
## 7. Plain-language owner communication
...
## 8. Relationship to YMB completeness rule
...
KW001_REMAINING_KWORK_WORK_RESTATE_EVERY_STEP = true
