...
Before **ANY material KW-002 action**, including preparation, planning, research, Work prompt drafting, Work execution, Bridge/provider work, analysis, QA, acceptance, publication, cursor movement, roadmap movement or owner-facing major report:
...
Before a major step/substep starts, record at minimum:

```text
LIVE_REMOTE_HEAD
CURRENT_ACTION
RULES_READ_IN_FULL = [exact current paths]
JOB_STATE_READ = [exact current paths]
...
## 8. Post-execution / acceptance gate
...
## 10. Anti-regression statement that must remain visible
...
## 11. Mandatory authorities to include when applicable
