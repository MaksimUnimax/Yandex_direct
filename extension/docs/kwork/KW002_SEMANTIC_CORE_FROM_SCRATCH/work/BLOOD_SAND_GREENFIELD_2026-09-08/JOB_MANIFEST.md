...
Status: **STEP 03 ACQUISITION COMPLETE / RAW RECOVERY 71/79 / STEP 04 BLOCKED**
...
CURRENT_DURABLE_FEED_FORWARD = 71/79
...
The historical manifest independently records run 40 as 569 results / 19 associations / totalCount 37900, matching the current direct remote carrier checks. The historical request identity remains preserved separately.
...
Historical provider economics are preserved separately from the current recovery cost; do not overwrite historical request accounting with the nine new recovery observations.
