...
Status: **STEP 03 ACQUISITION COMPLETE / RAW RECOVERY 74/79 / STEP 04 BLOCKED**
...
CURRENT_DURABLE_FEED_FORWARD = 74/79
...
The historical manifest independently records run 43 as 593 results / 15 associations / totalCount 37752, matching the current direct remote carrier checks. The historical request identity remains preserved separately.
...
Historical provider economics are preserved separately from the current recovery cost; do not overwrite historical request accounting with the twelve new recovery observations.