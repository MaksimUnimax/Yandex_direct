...
Status: **STEP 03 ACQUISITION COMPLETE / RAW RECOVERY 72/79 / STEP 04 BLOCKED**
...
CURRENT_DURABLE_FEED_FORWARD = 72/79
...
The historical manifest independently records run 41 as 952 results / 18 associations / totalCount 101545, matching the current direct remote carrier checks. The historical request identity remains preserved separately.
...
Historical provider economics are preserved separately from the current recovery cost; do not overwrite historical request accounting with the ten new recovery observations.
